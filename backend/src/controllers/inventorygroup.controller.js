...
async function getGroupsForInventory(req, res) {
  try {
    const groups = await inventoryGroupService.getGroupsForInventory(
...
async function getInventoryForGroup(req, res) {
  try {
    const inventory = await inventoryGroupService.getInventoryForGroup(
      req.params.groupId,
    );
...
async function addMapping(req, res) {
  try {
    const mapping = await inventoryGroupService.addGroupToInventory(
...
async function removeMapping(req, res) {
  try {
    await inventoryGroupService.removeGroupFromInventory(
      req.params.inventoryId,
      req.params.groupId,
    );
    res.status(204).send();
